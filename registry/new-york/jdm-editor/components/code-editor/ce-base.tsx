...
const editorTheme = (isDark = false) =>
  isDark ? zenHighlightDark : zenHighlightLight;
...
export const CodeEditorBase = React.forwardRef<
  CodeEditorBaseRef,
  CodeEditorBaseProps
>(
  (
      ...
      const editorView = new EditorView({
        ...
        state: EditorState.create({
          ...
          selection: initialSelection
            ? {
                head: initialSelection.head
                  ? clampSelection(initialSelection.head)
                  : undefined,
          ...
          extensions: [
            ...
            compartment.updateListener.of(
              updateListener(onChange, onStateChange),
            ),
            compartment.theme.of(editorTheme(resolvedTheme === "dark")),
            compartment.placeholder.of(
    ...
    useEffect(() => {
      ...
      codeMirror.current.dispatch({
        effects: compartment.theme.reconfigure(
    ...
    useEffect(() => {
      ...
      codeMirror.current.dispatch({
        effects: compartment.placeholder.reconfigure(
          placeholder ? placeholderExt(placeholder) : [],
        ),
      });
    }, [placeholder]);
    ...
    useEffect(() => {
      ...
      codeMirror.current.dispatch({
        effects: compartment.readOnly.reconfigure(
    ...
    useEffect(() => {
      ...
      codeMirror.current.dispatch({
        effects: compartment.updateListener.reconfigure(
          updateListener(onChange, onStateChange),
        ),
      });
    ...
    useEffect(() => {
      ...
      codeMirror.current.dispatch({
        effects: compartment.userProvided.reconfigure(
          extension?.({ type }) ?? [],
        ),
      });
    }, [extension, type]);
    ...
    useEffect(() => {
      ...
      codeMirror.current.dispatch({
        effects: updateExpectedVariableTypeEffect.of(
          createVariableType(expectedVariableType),
        ),
      });
    }, [expectedVariableType]);
...
CodeEditorBase.displayName = "CodeEditorBase";
