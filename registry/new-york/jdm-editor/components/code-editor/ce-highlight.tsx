...
export const CodeHighlighter = React.forwardRef<
  HTMLDivElement,
...
>(
  (
    ...
    useEffect(() => {
      injectStyles(isDark ? "dark" : "light");
    }, [isDark]);
    ...
    useEffect(() => {
      ...
      startTransition(() => {
        const types = match([type, variableType])
          .with(["unary", P.nonNullable], () =>
            createVariableType(variableType).typeCheckUnary(value),
          )
          .with(["standard", P.nonNullable], () =>
            createVariableType(variableType).typeCheck(value),
          )
          .otherwise(() => []);
    ...
    return (
      <div
        ...
        {...props}>
        <div className={cn("cm-editor")} data-severity={diagnosticSeverity}>
          <div className={cn("cm-scroller")}>
            <div
              className={cn("cm-content", "cm-lineWrapping")}
            ...
            <div
              ...
              <ul className="cm-tooltip-lint cm-tooltip-section">
                {tooltip.diagnostics.map((diagnostic, idx) => (
                  <li
                    key={idx}
                    className={`cm-diagnostic cm-diagnostic-${diagnostic.severity}`}>
                    ...
                    {diagnostic.source && (
                      <div className="cm-diagnosticSource">
                        {diagnostic.source}
                      </div>
                    )}
                  </li>
