...
type EditorState =
  | { type: "lazy" }
  | { type: "edit"; initialSelection?: CodeEditorProps["initialSelection"] };

export const CodeEditor = React.forwardRef<CodeEditorRef, CodeEditorProps>(
  (
    { lazy = false, value = "", type = "standard", disabled, onBlur, ...props },
    ref,
  ) => {
    const [editorState, setEditorState] = useState<EditorState>(() =>
      lazy ? { type: "lazy" } : { type: "edit" },
    );
    ...
    const handleClick = useCallback(
      (event: React.MouseEvent) => {
        ...
        const selection = containerRef.current
          ? (getCursorPositionFromClick(event, containerRef.current) ??
            undefined)
          : undefined;
    ...
    const handleFocus = useCallback(
      (_event: React.FocusEvent<HTMLDivElement, HTMLDivElement>) => {
        ...
        setEditorState({
          type: "edit",
          initialSelection: { anchor: value.length },
        });
      },
